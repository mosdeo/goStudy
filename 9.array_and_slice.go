...
func main() {
	...
	fmt.Println(arr)
	fmt.Println(slice)
	fmt.Println(reflect.TypeOf(arr))   //[10]int 型別有固定長度就是 array，比較沒彈性
	fmt.Println(reflect.TypeOf(slice)) //[]int 型別無固定長度就是 slice，比較有彈性

	// Slice copy
	// 這邊還不能理解，很玄，不知道 reflect.Value 從哪裡來的
	s1 := []int{0, 1, 2, 3, 4}
	s2 := reflect.ValueOf(s1)
	fmt.Println(s1)
	fmt.Println(s2)                 //與s1相同
	fmt.Println(reflect.TypeOf(s1)) //[]int
	fmt.Println(reflect.TypeOf(s2)) //reflect.Value
