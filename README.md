...
- go get 掃雷: 
  1. 中國大陸用 go get 幾乎都會 timeout，要改用: ``export GOPROXY=https://goproxy.cn」``
  2. 「cannot use path@version syntax in GOPATH mode」的對策：``export GO111MODULE=on`` 
...
- 解 ToDo
  1. 內建 Testing 進階：玩過 go 原生套件的 benchmark，與 test 是同一組的，使用方法大部分相同。「go test -test.bench=".*"」測試所有已經寫的 benchmark 方法。
